...
window.addEventListener('keydown', function(e) {
    for(let i = 0; i < keys.length; i++) {
        if(e.key == keys[i].getAttribute('keyname' ) || e.key == keys[i].getAttribute('lowerCaseName')) {
            ...
            console.log(keys[i].getAttribute('keyname'));
        }
        if(e.code == 'Space') {
...
//que el fondo vaya cambiando de color aleatoriamente cada 5 segundos
...
setInterval(() => {
    ...
    keyboard_lights.style.background = "#" + randomColor;
    //con transition
